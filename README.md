...
The final clean local headless Chrome gate recorded **0.60 ms desktop main-loop work p95** against a 20 ms regression budget and **0.70 ms mobile DPR 2 work p95** against a 25 ms budget. Browser pacing is reported separately so a hosted runner's scheduling does not masquerade as game workload. These figures are reproducible automation evidence from the current development machine, not physical-device certification.
