# Moto Rush X3 Documentation Hub

This directory is organized as the permanent visual and engineering record for the game. Start here when reviewing a release or picking development back up.

## Latest release candidate

- [v1.8.1 — Fast Failure, Great Finish](releases/v1.8.1.md): original Finish Forge receipt, authoritative arithmetic, itemized score ledger, PB/tie decisions, guarded routes, keyboard/touch/gamepad parity, semantic dialog, reset ownership, complete local/hosted gates, four inspected captures, compatibility boundary, and explicit delivery limits.
- [Performance and interruption QA](qa/PERFORMANCE.md): exact v1.8.1 ordinary, Finish Forge, and preserved live-crash browser profiles; budgets, harness behavior, input matrices, reproduction commands, and limitations.
- [Golden run QA](qa/GOLDEN_TAPES.md): all 16 v1.8.1 build-refreshed repository recovery tapes, exact unchanged v1.8.0 outcomes, reproduction commands, and limitations.
- [Screenshot archive](screenshots/README.md): versioned desktop, gameplay, debug, proof, performance, rotation, accessibility, and mobile captures.

## Archived release candidates

- [v1.8.0 — Crash Theater](releases/v1.8.0.md): original code-native 17-part Splitline rig, ten cause profiles, detached swept contact, bounded impact records, seeded feedback, Reduced Motion invariants, complete local/hosted gates, and four inspected captures.
- [v1.7.0 — Vector Weave](releases/v1.7.0.md): original Kinetic Loom force fields, a new teaching route, swept field detection, collision-aligned proxies, four inspected screenshots, compatibility refresh, and exact local/hosted gate evidence.
- [v1.6.0 — Smooth Ride](releases/v1.6.0.md): bounded effects, live frame telemetry, interruption-safe input, and left-handed controls.
- [v1.5.0 — Gold Standard](releases/v1.5.0.md): repository-owned Gold Runs, trigger-controlled platforms, collision proxy tooling, and the v1.5 visual record.

## Engineering reference

- [Project state](PROJECT_STATE.md): canonical branch/build handoff, current evidence, risks, and immediate priorities.
- [Architecture](ARCHITECTURE.md): runtime boundaries, deterministic order, compatibility identities, and extension rules.
- [Production roadmap](../ROADMAP.md): the complete 30-update plan and acceptance status.
- [Release history](../CHANGELOG.md): permanent chronological change record.
- [Feel specification](../design/FEEL_SPEC.md): physics and feedback targets.
- [Asset inventory](../design/assets.csv): art-production tracking.

## Store presentation

`cover.png` is the repository hero. The current store-style gallery lives under [`screenshots/v1.8.1/`](screenshots/v1.8.1/README.md) and contains inspected Finish Forge desktop hero, focused-action, mobile DPR 2, and Reduced Motion captures. The inspected [`screenshots/v1.8/`](screenshots/v1.8/README.md) Crash Theater and [`screenshots/v1.7/`](screenshots/v1.7/README.md) Vector Weave galleries remain preserved instead of being overwritten.
