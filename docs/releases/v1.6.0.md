...
The latest standalone browser run used the repository harness and a local system-installed Chrome in headless mode. Service workers were blocked, each profile warmed for 750 ms, telemetry was reset, and the next 3 seconds filled the bounded rolling window.

| Profile | DPR | p50 | p95 | p99 | p95 budget | Result |
|---|---:|---:|---:|---:|---:|---|
| Desktop 1280 × 720 | 1 | 0.60 ms | 8.12 ms | 105.10 ms | < 20 ms | Pass |
| Mobile 390 × 844 | 2 | 2.50 ms | 11.21 ms | 13.30 ms | < 25 ms | Pass |

The very high headless FPS implied by these frame intervals is not a display-refresh claim. It is useful repeatable local gate evidence, but it does not predict sustained performance on a thermally constrained phone. See [Performance and interruption QA](../qa/PERFORMANCE.md) for the full method, invariants, input matrix, interpretation, and limitations.
