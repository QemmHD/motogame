...
This document describes the unmerged `v1.8.0` Crash Theater release-candidate architecture as it exists on 2026-07-14. The game is a static, framework-free Canvas 2D application. `public/` is the complete deployment boundary. Crash Theater is presentation-only: replay schema `1`, `physics-4`, and `course-4` remain current, while build/cache identity advances because player-visible modules and presentation changed. Older build tapes remain explicitly incompatible even when their authoritative physics/course identities match.
...
Keep authoritative simulation behavior in `physics.js`, `rules.js`, `kinematics.js`, `force-zones.js`, `run-session.js`, or `replay.js`. Keep browser-neutral presentation simulation/policy in `ragdoll.js`, `crash-contact.js`, and `crash-presentation.js`; keep other browser-neutral infrastructure in `debug-proxies.js`, `effect-pool.js`, `input-state.js`, and `perf-metrics.js`. `game.js` adapts browser events, selects/records commands, advances `run-session.js`, and turns returned events into pooled presentation, sound, persistence, telemetry, and UI. Crash contact, cause impulses, ragdoll state, camera policy, effect seeds, and performance measurements are deliberately not authoritative run state.
...
9. `game.js` consumes plain flips, landings, score records, blasts, Kinetic Loom applications/entries, near misses, platform activation, checkpoint, crash, finish, and respawn events to drive audio, haptics, pooled particles/popups/tracks, camera, ragdoll, persistence, and UI without mutating authoritative arithmetic. The crash event carries a detached reason copy.
...
11. A finish finalizes or verifies the authoritative session snapshot. Rendering and Crash Theater presentation read state but do not advance authoritative bike/rules/platform ticks. After the frame, `perf-metrics.js` records frame duration, fixed ticks, accumulator backlog, clamped/dropped time, effect counts/capacity, and viewport state into its fixed ring.
...
3. `kinematics.js`, `force-zones.js`, `run-session.js`, `debug-proxies.js`, `replay.js`, `ragdoll.js`, `crash-contact.js`, `crash-presentation.js`, `effect-pool.js`, `input-state.js`, and `perf-metrics.js` also remain DOM-free and executable in Node tests.
